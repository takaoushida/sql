...
laglead_add as(
    select
        *,
        lag(period_month,1) over(partition by stock_code order by release_date) as last_period_month,
        lead(period_month,1) over(partition by stock_code order by release_date) as next_period_month
    from
        spreadsheet_add
    where
        refine_flg is null and omit_flg is null
),
errors as(
    select
        distinct stock_code
    from
        laglead_add
    where
        period_month != last_period_month and last_period_month = next_period_month
)
select
    t1.*
from
