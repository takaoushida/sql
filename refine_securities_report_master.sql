...
create or replace table temp_folder.all_summary_title
cluster by stock_code as(
select
    distinct
    release_date,
...
space_cut as(--漢字の数値を置換
    ...
    where
        ...
        or (
            not regexp_contains(title,r'資料|訂正|変更|再登録|追加|超えること|差替|キャッシュ・フロー計算書|DATABOOK|Q&A|PDF|について|お知らせ|サマリー|高い関心|データ集|推移表|~|FAQ|記者会見|報告会|取り組み|説明|書き起こし|決算説明会|ダイジェスト')--xbrlがなく、これらのキーワードを含んでいないなら決算短信
...
spreadsheet_add as(
    select
        ...
        case when t3.stock_code is not null then 1 end as xbrl_less_known_flg
    from
...
nonrefine_next_running as(--次回公開日を訂正行にも伝達
    select
        *,
        max(nonrefine_next_release_date) over(partition by stock_code,period,quarter) as running_nonrefine_next_release_date
    from
        nonrefine_next_add
),
release_date_refine as(--訂正の場合、次回公開日より後に出た
    select
        * ,
        case --訂正を除いた次回公開日が公開日より小さい場合(つまり次期公開の方が早い場合null)
            when refine_flg is not null and running_nonrefine_next_release_date <= release_date then null
            else release_date
        end as join_start_date
    from
        nonrefine_next_running
),
row_number_add as(--row_numberはjoin_start_dateがnullだとcountが進まない
    select
        *,
        count(join_start_date) over(partition by stock_code order by release_date,period,quarter,refine_flg) as row_number
    from
        release_date_refine
),
only_start_date as (
...
self_join as(
    select
        t1.*,
        t2.join_start_date as join_end_date,
    from
        row_number_add as t1
    left join
        only_start_date as t2
        on t1.stock_code = t2.stock_code and t1.row_number +1 = t2.row_number
),
end_add as(
    select
        * except(join_start_date,join_end_date),
        case
...
select
    * except(row_number,nonrefine_next_release_date,running_nonrefine_next_release_date),
from
    end_add
)
